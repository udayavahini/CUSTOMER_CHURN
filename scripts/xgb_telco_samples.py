...
def plot_svd_errors(preprocessor, model, X_train, X_test, y_test, title: str, out_path: Path) -> None:
    ...
    Z_test = svd.fit(Xt_train_s).transform(Xt_test_s)
...
def plot_top_feature_importances_xgb(pipe: Pipeline, out_path: Path, top_n: int = 20) -> None:
    ...
    plt.barh(s.index.astype(str), s.values, color=COLORS["blue"])
...
def evaluate_one_split(df_raw: pd.DataFrame, train_size: float, balance_method: str, out_dir: Path) -> dict:
    ...
    RocCurveDisplay.from_predictions(y_test, y_proba, name="XGBoost", color=COLORS["blue"])
    ...
    # SVD 2D scatter highlighting errors (similar to your SVD visuals)
    plot_svd_errors(
    ...
    # Feature importances
    plot_top_feature_importances_xgb(pipe, out_dir / tag / "feature_importances_top20.png", top_n=20)
...
def main() -> None:
    ...
    metrics_path = out_dir / "xgb_metrics_summary.csv"
